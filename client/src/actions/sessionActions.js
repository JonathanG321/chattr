...
const createSession = (userCredentials) => (dispatch) =>
  Session.create(userCredentials).then(
    (user) => dispatch({ type: SIGN_IN, payload: user }).payload,
  );
...
const createUser = (newUser) => (dispatch) =>
  User.create(newUser).then((user) => dispatch({ type: SIGN_UP, payload: user }).payload);
